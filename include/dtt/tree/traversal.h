...
namespace dtt::tree
{
    ...
    */
    ...
    /**
     * @brief Determines whether two nodes should be accepted for interaction based on the opening angle theta.
     * @param a First node.
     * @param b Second node.
     ...
     */
    bool accept(const Node &a, const Node &b, const double theta) {
        if (&a == &b)
            return false;
        const double dx = a.com_x - b.com_x;
        const double dy = a.com_y - b.com_y;
        const double dist_sq = dx * dx + dy * dy + 1e-18;
        const double size = std::max(a.box.max_extent(), b.box.max_extent());
        if (theta <= 0.0)
            return false;
     ...
     */
    bool prune(const Node &a, const Node &b, const double cutoff) {
        if (cutoff <= 0.0)
            return false;
    ...
    void dual_tree_traversal(const Tree &tree, const double theta, LeafFunc on_leaf_pair, AcceptFunc on_accepted_pair) {
